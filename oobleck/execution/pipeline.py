...
class PipelineExecution:
    ...
    def load_microbatch(self, buffer_id: int):
        ...
        if self.pipeline.is_first_stage():
            ...
            print(f"size: {s}")
            inputs = (torch.randint(s, dtype=torch.int64, requires_grad=False).to(self.pipeline.device), 
                        torch.ones(s, dtype=torch.int64, requires_grad=False).to(self.pipeline.device),
                        torch.randint(s, dtype=torch.int64, requires_grad=False).to(self.pipeline.device))
