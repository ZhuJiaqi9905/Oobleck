...
std::vector<PipelineTemplate> PipelineTemplateGenerator::create_pipeline_templates_serial(
    std::shared_ptr<LayerExecutionResults> layer_execution_results,
    const std::tuple<int, int>& num_nodes,
    const int num_gpus_per_node) {
  std::cout << "Create pipeline template serial" << std::endl;
...
#endif
  std::map<int, std::vector<std::shared_ptr<DCExecutionResult>>> tasks;
  for(int i = min_num_nodes; i <= max_num_nodes; i++) {
    ...
    std::vector<std::shared_ptr<DCExecutionResult>>
        num_node_tasks;
    for (int num_stages = min_num_stages; num_stages <= max_num_stages;
         num_stages++) {
      num_node_tasks.emplace_back(divide_and_conquer_serial(
  ...
  for (auto&& num_node_tasks : tasks) {
    ...
    std::vector<std::shared_ptr<DCExecutionResult>> results = std::move(num_node_tasks.second);
...
PipelineTemplateGenerator::create_pipeline_templates(
    ...
    const std::tuple<int, int>& num_nodes,
    const int num_gpus_per_node) {
  std::cout << "Create pipeline templates" << std::endl;
  int min_num_nodes = std::get<0>(num_nodes);
  int max_num_nodes = std::get<1>(num_nodes);
#ifdef PYBIND11_MODULE
  // Release GIL
...
PipelineTemplateGenerator::divide_and_conquer_serial(
    std::shared_ptr<LayerExecutionResults> layer_execution_results,
      ...
      for (int num_gpus_left :
        ...
        for (int num_stages_left :
          ...
          if (it != dc_cache_.end()) {
            ...
            result_left = divide_and_conquer_serial(
          ...
          if (it != dc_cache_.end()) {
            ...
            result_right = divide_and_conquer_serial(
