import os
import re
DIR = "/workspace/Oobleck/tmp/simulate_livepipe_size_logs"
        ...
        with open(f"{DIR}/{file_dir}/172.21.0.42-2220.log") as f:
            for line in f:
                if '(GPU, Rank 0 | Time(averaged 2 times)' in line:
                    # 使用正则表达式提取所需信息
                    match = re.search(r'Time\(averaged 2 times\) = ([\d.]+) ms \| trans_size = ([\d ]+) B', line)
                    if match:
                        # 提取的信息
                        time_taken = match.group(1)  # 716.29 ms
                        trans_size = match.group(2)
                        res.setdefault(model, []).append((ori_nodes, curr_nodes, time_taken, trans_size))
                        break
    for l in res.values():
        l.sort(key=lambda x: x[0])
    for model, infos in res.items():
        for info in infos:
            print(f"{model} {info[0]} -> {info[1]}: {info[2]} ms | {float(info[3]) / (1024 * 1024 * 1024) } GB")
